$(document).ready(function() {
    ...
    function setFormState(state) {
        ...
        $('input', dataForm).not('#edit_id').not('#emer_lights_installation_id').prop('disabled', is_disabled);
    ...
    function fetchData(query = '') {
        ...
        $.ajax({
            url: '../../core/emer_lights_installation_handler.php',
            type: 'GET',
    ...
    clearButton.on('click', function() {
        dataForm[0].reset();
        editIdField.val('');
        $('#emer_lights_installation_id').val('');
    ...
    addNewButton.on('click', function() {
        ...
        $('#emer_lights_installation_id').prop('disabled', false).focus();
    });

    editButton.on('click', function() { 
        setFormState('enabled'); 
        $('#emer_lights_installation_id').prop('disabled', true);
    });
    ...
    tableBody.on('click', '.edit-btn', function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $('#emer_lights_installation_id').val(data.id);
                $('#name_type').val(data.name_type);
