$(document).ready(function() {
    ...
    const hospitalIdField = $('#hospital_id');
    ...
    function setFormState(state) {
        ...
        $('input', dataForm).not('#edit_id').not('#hospital_id').prop('disabled', is_disabled);
    ...
    function fetchData(query = '') {
        tableBody.html('<tr><td colspan="6" class="text-center">กำลังโหลดข้อมูล...</td></tr>');
        $.ajax({
            url: '../../core/hospital_handler.php',
            type: 'GET',
            ...
            success: function(response) {
                ...
                if (response.data && response.data.length > 0) {
                    response.data.forEach(item => {
                        ...
                        const row = `
                            <tr data-id="${item.id}">
                                ...
                                <td class="text-center">${item.hosp_id}</td>
                                <td>${item.hosp_name}</td>
                                <td class="text-center">${item.sub_district_id}</td>
                    ...
                    tableBody.html('<tr><td colspan="6" class="text-center">ไม่พบข้อมูล</td></tr>');
                }
            }
        });
    }
    ...
    addNewButton.on('click', function() {
        ...
        $('#hosp_id').focus(); // เปลี่ยนให้ focus ที่ช่อง รหัส (5 หลัก)
    });
    ...
    tableBody.on('click', '.edit-btn', function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                hospitalIdField.val(data.id);
                $('#hosp_id').val(data.hosp_id);
                $('#hosp_name').val(data.hosp_name);
