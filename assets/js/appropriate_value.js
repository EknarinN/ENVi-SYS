$(document).ready(function() {
    ...
    function setFormState(state) {
        ...
        $('input:not(#appropriate_value_id)', dataForm).prop('disabled', is_disabled);
    ...
    function fetchData(query = '') {
        ...
        $.ajax({
            url: '../../core/appropriate_value_handler.php',
            type: 'GET',
            ...
            success: function(response) {
                ...
                if (response.data && response.data.length > 0) {
                    response.data.forEach(item => {
                        const statusBadge = item.usage_id == '1' ? '<span class="badge bg-success">ใช้งาน</span>' : '<span class="badge bg-danger">ไม่ใช้งาน</span>';
                        const row = `<tr data-id="${item.id}">
                            <td class="text-center">${item.id}</td>
                            <td>${item.name_full_word}</td>
                            <td>${item.name_abbreviation_word}</td>
                            <td class="text-end">${parseFloat(item.appropriate_value_min).toFixed(2)}</td>
                            <td class="text-end">${parseFloat(item.appropriate_value_max).toFixed(2)}</td>
    ...
    tableBody.on('click', '.edit-btn', function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                $('#edit_id').val(data.id);
                $('#appropriate_value_id').val(data.id);
                $('#name_full').val(data.name_full_word);
                $('#name_abbr').val(data.name_abbreviation_word);
                $('#value_min').val(parseFloat(data.appropriate_value_min).toFixed(2));
                $('#value_max').val(parseFloat(data.appropriate_value_max).toFixed(2));
    ...
    tableBody.on('click', '.delete-btn', function() {
        ...
        if (confirm(`คุณแน่ใจหรือไม่ว่าต้องการลบข้อมูลรหัส ${id}?`)) {
            $.ajax({
                ...
                dataType: 'json',
                success: function(response) {
                    showAlert(response.message, response.status); // แก้ไขตรงนี้
