...
switch ($action) {
    case 'fetch':
        $query = $_GET['query'] ?? '';
        $sql = "SELECT * FROM data_waste_type";
        $params = [];
        if (!empty($query)) {
    ...
    case 'get_one':
        $id = $_GET['id'] ?? 0;
        $stmt = $pdo->prepare("SELECT * FROM data_waste_type WHERE id = ?");
        $stmt->execute([$id]);
        $data = $stmt->fetch();
    ...
    case 'save':
        ...
        $id = $is_edit ? $_POST['edit_id'] : $_POST['waste_type_id'];
        
        $sql = $is_edit
            ? "UPDATE data_waste_type SET name_type=?, usage_id=? WHERE id=?"
    ...
    case 'delete':
        $id = $_POST['id'] ?? 0;
        $stmt = $pdo->prepare("DELETE FROM data_waste_type WHERE id = ?");
        if ($stmt->execute([$id])) {
