...
switch ($action) {
    case 'fetch':
        $query = $_GET['query'] ?? '';
        $sql = "SELECT * FROM data_fire_extinguisher_type";
        $params = [];
        if (!empty($query)) {
    ...
    case 'get_one':
        $id = $_GET['id'] ?? 0;
        $stmt = $pdo->prepare("SELECT * FROM data_fire_extinguisher_type WHERE id = ?");
        $stmt->execute([$id]);
        $data = $stmt->fetch();
    ...
    case 'save':
        ...
        if (!$is_edit) {
            ...
            $stmt = $pdo->query("SELECT MAX(CAST(SUBSTRING(id, 4) AS UNSIGNED)) as max_id FROM data_fire_extinguisher_type");
            $max_id = $stmt->fetchColumn();
            $new_id_num = ($max_id ?? 0) + 1;
            $id = 'FET' . str_pad($new_id_num, 2, '0', STR_PAD_LEFT);
        }

        $sql = $is_edit
            ? "UPDATE data_fire_extinguisher_type SET name_type=?, usage_id=? WHERE id=?"
    ...
    case 'delete':
        $id = $_POST['id'] ?? 0;
        $stmt = $pdo->prepare("DELETE FROM data_fire_extinguisher_type WHERE id = ?");
        if ($stmt->execute([$id])) {
