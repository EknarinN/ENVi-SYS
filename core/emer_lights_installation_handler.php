...
switch ($action) {
    case 'fetch':
        $query = $_GET['query'] ?? '';
        $sql = "SELECT * FROM data_emer_lights_installation";
        $params = [];
        if (!empty($query)) {
    ...
    case 'get_one':
        $id = $_GET['id'] ?? 0;
        $stmt = $pdo->prepare("SELECT * FROM data_emer_lights_installation WHERE id = ?");
        $stmt->execute([$id]);
        $data = $stmt->fetch();
    ...
    case 'save':
        ...
        $id = $is_edit ? $_POST['edit_id'] : $_POST['emer_lights_installation_id'];
        
        $sql = $is_edit
            ? "UPDATE data_emer_lights_installation SET name_type=?, usage_id=? WHERE id=?"
    ...
    case 'delete':
        $id = $_POST['id'] ?? 0;
        $stmt = $pdo->prepare("DELETE FROM data_emer_lights_installation WHERE id = ?");
        if ($stmt->execute([$id])) {
